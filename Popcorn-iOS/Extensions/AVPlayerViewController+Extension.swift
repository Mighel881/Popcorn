...
public extension AVPlayerViewController {
    ...
    fileprivate func addSubtitleLabel() {
        ...
        guard let _ = subtitleLabel else {
            ...
            subtitleLabel?.font = UIFont.boldSystemFont(ofSize: UIDevice.current.userInterfaceIdiom == .pad ? 32.0 : 15.0)
            ...
            constraints = NSLayoutConstraint.constraints(withVisualFormat: "V:[l]-(20)-|", options: NSLayoutConstraint.FormatOptions(rawValue: 0), metrics: nil, views: ["l" : subtitleLabel!])
            contentOverlayView?.addConstraints(constraints)
